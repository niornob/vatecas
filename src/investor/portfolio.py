...
class SizingModel(ABC):
    ...
    @abstractmethod
    def size_position(
        self, signal: float, price: float, portfolio: "Portfolio"
    ) -> float:
        """
...
class Portfolio:
    ...
    sizing_model: SizingModel = FixedFractionalSizing(fraction=0.1)
    ...
    last_trade_time: dict[str, pd.Timestamp] = field(default_factory=dict)
    # time-series records
