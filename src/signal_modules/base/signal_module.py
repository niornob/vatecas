...
class MovingAverageSmoothing(SmoothingStrategy):
    ...
    def __init__(self):
        self.prediction_history = {}  # Store history per ticker
    ...
    def smooth(self, predictions: np.ndarray, window: int) -> np.ndarray:
        ...
        for i, pred in enumerate(predictions):
            ...
            if ticker_key not in self.prediction_history:
                self.prediction_history[ticker_key] = []
            
            # Add current prediction to history
            self.prediction_history[ticker_key].append(pred)
            
            # Keep only the required window length
            if len(self.prediction_history[ticker_key]) > window:
                self.prediction_history[ticker_key] = self.prediction_history[ticker_key][-window:]
            
            # Calculate moving average
            smoothed[i] = np.mean(self.prediction_history[ticker_key])
...
class SignalModule:
    ...
    def generate_signals(self, data: Dict[str, pd.Series]) -> SignalResult:
        ...
        """
        # Step 1: Get predictions from Oracle
        ...
        # Step 2: Apply smoothing to predictions
        smoothed_predictions = self.smoothing_strategy.smooth(
            raw_predictions, self.smoothing_window
        )
        
        # Step 3: Convert predictions to directional signals
        # Positive predictions -> buy signals, negative -> sell signals
        directional_signals = np.sign(smoothed_predictions) * np.abs(smoothed_predictions)
        
        # Step 4: Apply volatility-based position sizing
        vol_adjusted_signals, vol_scaling = self.vol_adjuster.adjust_for_volatility(
            directional_signals, asset_covariance
        )
        
        # Step 5: Normalize signals to [-1, +1] range
        final_signals = self._normalize_signals(vol_adjusted_signals)
    ...
    def _normalize_signals(self, signals: np.ndarray) -> np.ndarray:
        ...
        # Convert to ranks (0 to n-1)
        ranks = np.argsort(np.argsort(signals))
        
        # Normalize ranks to [-1, +1]
        if len(ranks) > 1:
            normalized = 2.0 * ranks / (len(ranks) - 1) - 1.0
        else:
            normalized = np.array([0.0])
        
        # Preserve sign from original signals
        signs = np.sign(signals)
        signs[signs == 0] = 1  # Handle zero predictions
    ...
    def _calculate_confidence(
        ...
        """
        # Consistency between raw and smoothed predictions
        consistency = 1.0 - np.abs(raw_predictions - smoothed_predictions) / (np.abs(raw_predictions) + 1e-6)
        ...
        strength_confidence = np.abs(smoothed_predictions) / (np.max(np.abs(smoothed_predictions)) + 1e-6)
